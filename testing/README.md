...
|`test6()`|Generate a temporary task `ZZZTestTask.cl` and then load all tasks in the `tasks` directory. Then modify the temporary task's configuration file with missing semicolons on every entry. Then, attempt to reload all the tasks. The `EventReporter` should record 5 informational events and one event indicating that a configuration file could not be parsed. The original `ZZZTestTask` task should still be loaded in the `Scheduler` un changed.|`Scheduler::reload_all_tasks(void)`<br/>`EventReporter::get_n_events(void)`<br/>`Scheduler::get_n_tasks(void)`|<span style="color:green">Passed|
