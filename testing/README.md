...
|Testcase|Description|Part Tested|Status|
|-|-|-|-|
|`test1()`|Initialize `Scheduler` object and verify its data members were properly zeroed out.  The `EventReporter` should not register any events.|`Scheduler::Scheduler_init(void)`<br/>`Scheduler::Scheduler_delete(void)`<br/>`Scheduler::get_n_tasks(void)`<br/>`Scheduler::get_current_path(void)`<br/>`EventReporter::get_n_events(void)`|<span style="color:green">Passed|
|`test2()`|Verify that the `exec_path` data member is updated to the directory path where the test build is located. The `EventReporter` should not register any events.|`Scheduler::obtain_exec_path(void)`<br/>`Scheduler::get_current_path(void)`<br/>`EventReporter::get_n_events(void)`|<span style="color:green">Passed|
|`test3()`|Load tasks from `tasks` directory. Then, remove one task from `Scheduler`. After a successful call, there should only be 1 task left in the `Scheduler`. The `EventReporter` should register exactly 3 events after loading both tasks and removing one.|`Scheduler::remove_task(std::string&)`<br/>`EventReporter::get_n_events(void)`|<span style="color:green">Passed|
|`test4()`|Load tasks from `tasks` directory. Then, remove two tasks from `Scheduler`, one by one. After a successful call, there should be 0 tasks in the `Scheduler`. The `EventReporter` should register exactly 4 events after loading and removing both tasks.|`Scheduler::remove_task(std::string&)`<br/>`EventReporter::get_n_events(void)`|<span style="color:green">Passed|
|`test5()`|Load tasks from `tasks` directory. Then, attempt to remove one task that does not exist from `Scheduler`. The original 2 tasks should remain in the `Scheduler` unchanged. The `EventReporter` should register exactly 3 events after loading both tasks and failed attempt of removing one.|`Scheduler::remove_task(std::string&)`<br/>`EventReporter::get_n_events(void)`|<span style="color:green">Passed|
|`test6()`|Query a task name for a task that does exist in `Scheduler`. The call should return false. The `EventReporter` should register two events.|`Scheduler::task_exists(std::string&)`<br/>`EventReporter::get_n_events(void)`|<span style="color:green">Passed|
|`test7()`|Query a task name for a task that does not exist in `Scheduler`. The call should return false. The `EventReporter` should register two events.|`Scheduler::task_exists(std::string&)`<br/>`EventReporter::get_n_events(void)`|<span style="color:green">Passed|
|`test8()`|Generate `TestTask` and load it into the `Scheduler`. The `EventReporter` should report exactly 1 event after loading the task. Then query the loaded task validate its attributes. The `EventReporter` should not register any events when quering a task.|`Scheduler::get_task(std::string&)`<br/>`EventReporter::get_n_events(void)`|<span style="color:green">Passed|
|`test9()`|Load all tasks in `tasks` directory into the `Scheduler`. Then, attempt to query a task that does not exist in the `Scheduler`. The `EventReporter` should register 3 events with one of them being the attempt querying the task.|`Scheduler::get_task(std::string&)`<br/>`EventReporter::get_n_events(void)`|<span style="color:green">Passed|
...
|`test6()`|Generate a temporary task `ZZZTestTask.cl` and then load all tasks in the `tasks` directory. Then modify the temporary task's configuration file with missing semicolons on every entry. Then, attempt to reload all the tasks. The `EventReporter` should record 5 informational events and one event indicating that a configuration file could not be parsed. The original `ZZZTestTask` task should still be loaded in the `Scheduler` unchanged.|`Scheduler::reload_all_tasks(void)`<br/>`EventReporter::get_n_events(void)`<br/>`Scheduler::get_n_tasks(void)`|<span style="color:green">Passed|
...
|`test7()`|Generate a temporary task `ZZZTestTask.cl` and then load it. Then modify the temporary task's configuration file with missing semicolons on every entry. Then, attempt to reload the task. The `EventReporter` should record 2 events and one event indicating that a configuration file could not be parsed. The original `ZZZTestTask` task should still be loaded in the `Scheduler` unchanged.|`Scheduler::reload_task(void)`<br/>`EventReporter::get_n_events(void)`<br/>`Scheduler::get_n_tasks(void)`|<span style="color:green">Passed|
...
|`test2()`|Attempt to load task in the `Scheduler` by issuing the command `load task ls_test.cl invalid`. It should cause the `Scheduler` to report a warning event. Verify the event message, event time, and event type. |`CommandLine::parse_command(void)`<br/>`CommandLine::verb_load(std::vector<std::string>&)`|<span style="color:green">Passed|
...
### **CommandLine_verb_dump.cpp**

|Testcase|Description|Part Tested|Status|
|-|-|-|-|
|`test1()`|Issue the command `dump`. It should cause the `Scheduler` to report a warning event. Verify the event message, event time, and event type. |`CommandLine::parse_command(void)`<br/>`CommandLine::verb_dump(std::vector<std::string>&)`|<span style="color:green">Passed|
|`test2()`|Attempt to load task in the `Scheduler` by issuing the command `dump output ls invalid`. It should cause the `Scheduler` to report a warning event. Verify the event message, event time, and event type. |`CommandLine::parse_command(void)`<br/>`CommandLine::verb_dump(std::vector<std::string>&)`|<span style="color:green">Passed|
|`test3()`|Generate and load the `ZZZTestTask` temporary task. Let the task run and then issue the command `dump output ZZZTestTask`. The command should generate an output file. Verify informational event generated by creating the dump file, and verify the contents of the dump file. |`CommandLine::parse_command(void)`<br/>`CommandLine::verb_dump(std::vector<std::string>&)`<br/>`Scheduler::dump_task_output(std::string&)`|<span style="color:green">Passed|
|`test4()`|Attempt to get output dump from task that does not exist in the `Scheduler` by issuing the command `dump output invalid`. It should cause the `Scheduler` to report an error event. Verify the event message, event time, and event type. |`CommandLine::parse_command(void)`<br/>`CommandLine::verb_dump(std::vector<std::string>&)`<br/>`Scheduler::dump_task_output(std::string&)`|<span style="color:green">Passed|
|`test5()`|Issue the command `dump outputs`. It should cause the `Scheduler` to report a warning event. Verify the event message, event time, and event type. |`CommandLine::parse_command(void)`<br/>`CommandLine::verb_load(std::vector<std::string>&)`|<span style="color:green">Passed|
|`test6()`|Generate and load the `ZZZTestTask1` and `ZZZTestTask2` temporary tasks. Let the tasks run and then issue the command `dump outputs ZZZTestTask1 ZZZTestTask2`. The command should generate two output files. Verify informational events generated by creating the dump files, and verify the contents of the dump files. |`CommandLine::parse_command(void)`<br/>`CommandLine::verb_dump(std::vector<std::string>&)`<br/>`Scheduler::dump_task_output(std::string&)`|<span style="color:green">Passed|
|`test7()`|Attempt to generate output dump of two tasks in the `Scheduler` by issuing the command `dump outputs cat invalid`. It should cause the `Scheduler` only generate the output dump for the `ZZZTestTask` task but also report an event error on the second task name. Verify the events message, events time, and events type. |`CommandLine::parse_command(void)`<br/>`CommandLine::verb_dump(std::vector<std::string>&)`<br/>`Scheduler::dump_task_output(std::string&)`|<span style="color:green">Passed|
|`test8()`|Issue the command `dump events invalid`. It should cause the `Scheduler` to report a warning event. Verify the event message, event time, and event type. |`CommandLine::parse_command(void)`<br/>`CommandLine::verb_dump(std::vector<std::string>&)`|<span style="color:green">Passed|
|`test9()`|Load all tasks in the `tasks` directory into the `Scheduler`. Then, issue the command `dump events` to generate a dump file containing all the events recorded by the `Scheduler`. Verify the dump event message, event time, and event type. Also, verify the dump events file contents by comparing each of the recorded events in the actual `Scheduler`. |`CommandLine::parse_command(void)`<br/>`CommandLine::verb_dump(std::vector<std::string>&)`<br/>`Scheduler::dump_events(std::string&)`|<span style="color:green">Passed|
|`test10()`|Issue the command `dump invalid`. It should cause the `Scheduler` to report a warning event. Verify the event message, event time, and event type. |`CommandLine::parse_command(void)`<br/>`CommandLine::verb_dump(std::vector<std::string>&)`|<span style="color:green">Passed|

### **CommandLine_verb_help.cpp**

|Testcase|Description|Part Tested|Status|
|-|-|-|-|
|`test1()`|Issue the command `help`. It should print the full menu describing usage of all available verbs in the schduler. Verify output and command count. |`CommandLine::parse_command(void)`<br/>`CommandLine::verb_help(std::vector<std::string>&)`<br/>`CommandLine::help_msg(void)`<br/>`CommandLine::help_check_msg(void)`<br/>`CommandLine::help_remove_msg(void)`<br/>`CommandLine::help_load_msg(void)`<br/>`CommandLine::help_reload_msg(void)`<br/>`CommandLine::help_dump_msg(void)`|<span style="color:green">Passed|
...
|`test6()`|Issue the command `help dump`. It should print text describing usage of the `dump` verb. Verify output and command count. |`CommandLine::parse_command(void)`<br/>`CommandLine::verb_help(std::vector<std::string>&)`<br/>`CommandLine::help_dump_msg(void)`|<span style="color:green">Passed|
|`test7()`|Issue the command `help invalid`. It should cause the `Scheduler` to report a warning event. Verify the event message, event time, and event type. |`CommandLine::parse_command(void)`<br/>`CommandLine::verb_help(std::vector<std::string>&)`<br/>`CommandLine::help_msg(void)`|<span style="color:green">Passed|