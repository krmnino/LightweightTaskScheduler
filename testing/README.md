...
|`test7()`|Log an event and attempt to retrieve an event from `EventReporter` with an index equal to the number of events recorded. A warning-type event should be logged and returned with an appropiate message and update the event count.|`EventReporter::get_event_at(unsigned int)`|<span style="color:green">Passed|
|`test8()`|Log an event and attempt to retrieve an event from `EventReporter` with an index greater than the number of events recorded. A warning-type event should be logged and returned with an appropiate message and update the event count.|`EventReporter::get_event_at(unsigned int)`|<span style="color:green">Passed|
