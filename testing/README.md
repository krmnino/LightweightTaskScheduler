...
|`test2()`|Rename `tasks` directory to simulate it does not exist. The `EventReporter` should record 1 event after attempting to find the `tasks` directory. No task should be loaded in the loaded in the `Scheduler`.|`Scheduler::load_tasks_from_dir(void)`<br/>`EventReporter::get_n_events(void)`|<span style="color:green">Passed|
|`test3()`|Rename `scripts` directory to simulate it does not exist. The `EventReporter` should record 1 event after attempting to find the `scripts` directory. No task should be loaded in the `Scheduler`.|`Scheduler::load_tasks_from_dir(void)`<br/>`EventReporter::get_n_events(void)`|<span style="color:green">Passed|