...
|`test4()`|Load task in the scheduler and then issue the command `remove task invalid`. It should cause the scheduler to report a warning event. Verify the event message, event time, event type, and command count. |`CommandLine::parse_command(void)`<br/>`CommandLine::verb_remove(void)`<br/>`Scheduler::remove_task(std::string&)`|<span style="color:green">Passed|
