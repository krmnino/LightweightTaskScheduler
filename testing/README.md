...
|`test5()`|Issue the command `help reload`. It should print text describing usage of the `load` verb. Verify output and command count. |`CommandLine::parse_command(void)`<br/>`CommandLine::verb_help(std::vector<std::string>&)`<br/>`CommandLine::help_load_msg(void)`|<span style="color:green">Passed|
...
|`test7()`|Issue the command `help invalid invalid`. It should cause the `Scheduler` to report a warning event. Verify the event message, event time, and event type.|`CommandLine::parse_command(void)`<br/>`CommandLine::verb_help(std::vector<std::string>&)`|<span style="color:green">Passed|