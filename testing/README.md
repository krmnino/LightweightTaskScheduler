...
|`test8()`|Initialize `EventReporter` object and verify its data members were properly zeroed out.  The `EventReporter` should not register any events.|`EventReporter::EventReporter_init(void)`<br/>`EventReporter::EventReporter_delete(void)`<br/>`Scheduler::get_n_tasks(void)`<br/>`Scheduler::Scheduler_get_current_path(void)`<br/>`EventReporter::get_n_events(void)`|<span style="color:green">Passed|