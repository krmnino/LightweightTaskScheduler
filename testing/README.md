...
|`test7()`|Generate a temporary task `ZZZTestTask.cl` and then load it. Then modify the temporary task's configuration file with missing semicolons on every entry. Then, attempt to reload the task. The `EventReporter` should record 2 events and one event indicating that a configuration file could not be parsed. The original `ZZZTestTask` task should still be loaded in the `Scheduler` unchanged.|`Scheduler::reload_all_tasks(void)`<br/>`EventReporter::get_n_events(void)`<br/>`Scheduler::get_n_tasks(void)`|<span style="color:green">Passed|
